"""Headless local-only SVG bounds inspection and responsive gallery check."""
import json
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for width in (320,390,760,1040):
            page.set_viewport_size({'width':width,'height':850})
            page.goto((ROOT/'index.html').as_uri())
            page.wait_for_function('[...document.images].every(i=>i.complete&&i.naturalWidth>0)')
            result=page.evaluate('({scrollWidth:document.documentElement.scrollWidth,width:innerWidth,images:[...document.images].every(i=>i.complete&&i.naturalWidth>0),links:[...document.querySelectorAll("article>a")].every(a=>a.getAttribute("href").endsWith(".svg"))})')
    ...
    assert all(f.get('scrollWidth',0)<=f.get('width',0) and f.get('images',True) and f.get('links',True) for f in findings),'Gallery layout'
