...
import { ClosedDayCross } from './closed-day-cross';
...
export function PhoneCalendar({ booker, csrf, active, editable, onSaved, onRefresh, refreshing, onCancel, cancelling }: {
  ...
  const bookingOff = (date: string) => Boolean(values && !state(date).enabled);
  const dateStateClass = (date: string) => `${bookingOff(date) ? 'booking-off-key' : ''} ${closed(date) ? 'closed-key' : ''}`.trim();
  ...
  const planDates = dates.filter(date => closed(date) || plans.some(day => day.date === date));
  ...
  return <section className="view-page phone-calendar" aria-label="Calendar">
    ...
    {(mode === 'month' || mode === 'fortnight') && <><div className="calendar-weekdays" aria-hidden="true">{['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => <span key={day}>{day}</span>)}</div><div className="calendar-month">{dates.map(date => <button type="button" key={date} aria-label={`${label(date)}, ${values ? state(date).enabled ? 'booking on' : 'booking off' : 'settings not loaded'}${closed(date) ? ', practice rooms closed' : ''}${edits[date] ? ', unsaved changes' : ''}`} aria-pressed={selecting ? selection.includes(date) : focus === date} className={`${focus === date ? 'focused' : ''} ${selection.includes(date) && selecting ? 'selected' : ''} ${bookingOff(date) ? 'day-off' : ''} ${closed(date) ? 'day-closed closed-day-surface' : ''} ${date.slice(0, 7) !== anchor.slice(0, 7) ? 'outside-month' : ''}`} onClick={() => { setFocus(date); if (selecting && date >= today) setSelection(current => current.includes(date) ? current.filter(item => item !== date) : [...current, date]); }}>{closed(date) && <ClosedDayCross />}<span>{Number(date.slice(8))}</span><small>{edits[date] ? '✎' : closed(date) ? '' : events(date).length ? '•' : state(date).time ? '◷' : ''}</small></button>)}</div><p className="calendar-legend">● Event · crossed date: booking off · red crossed date: rooms closed · ◷ custom time</p></>}
    {(mode === 'week' || mode === '3days') && <div className="calendar-timeline" aria-label="Day timelines, scroll horizontally for more dates">{dates.map(date => <div className="timeline-day" key={date}><button type="button" className={dateStateClass(date)} onClick={() => setFocus(date)}>{label(date, true)}</button><div className={`timeline-track${closed(date) ? ' closed-day-surface' : ''}`} data-date={date}>{closed(date) && <><ClosedDayCross /><span className="timeline-closure-label">Practice rooms closed</span></>}{Array.from({ length: 9 }, (_, i) => <span key={i} className="timeline-hour" style={{ top: i * 72 }}>{`${7 + i * 2}:00`}</span>)}{events(date).map((event, index) => <button key={index} type="button" className={`timeline-event ${event.is_reservation ? '' : 'class-event'}`} style={{ top: Math.max(0, (minutes(event.start_time) - 420) * .6), minHeight: 30, height: Math.max(30, (minutes(event.end_time) - minutes(event.start_time)) * .6) }} onClick={() => setDetail(event)}>{event.start_time}–{event.end_time}<br />{event.room}<br />{event.is_reservation ? 'Booked' : event.title}</button>)}{plans.filter(day => day.date === date).flatMap(day => selectedPlanSessions(day)).filter(candidate => candidate.potential_minutes > 0).map((candidate, index) => <button key={`potential-${index}`} type="button" className="timeline-potential" style={{ top: Math.max(0, (minutes(candidate.start_time) - 420) * .6), height: Math.max(30, (minutes(candidate.end_time) - minutes(candidate.start_time)) * .6) }} onClick={() => { setFocus(date); setMode('plan'); }}>{candidate.room}<br />{candidate.start_time}–{candidate.end_time}<br />Potential</button>)}</div></div>)}</div>}
    {mode === 'plan' && <div>
      {!plans.length && <p className="quiet-notice">{booker.plan.stale ? 'Plan needs refreshing.' : 'No checked plan available for this period.'}</p>}
      {planDates.map(date => {
        const day = plans.find(day => day.date === date);
        return <article key={date} data-date={date} className={`calendar-plan${closed(date) ? ' closed-day-surface' : ''}`}>
          {closed(date) && <ClosedDayCross />}
          <h3 className={dateStateClass(date)}>{label(date, true)}</h3>
          {closed(date) && <p className="closure-label">Practice rooms closed</p>}
          {day && <><p>Target {day.target_minutes / 60} hours · booked {day.existing_minutes / 60} hours</p><p>{day.reason}</p>
            {selectedPlanSessions(day).map((candidate, index) => <div key={index} className="potential-session">{candidate.room} · {candidate.start_time}–{candidate.end_time}<br />{candidate.potential_minutes > 0 ? `${candidate.potential_minutes} potential minutes · not reserved` : 'Already booked'}<p>{candidate.reason}</p></div>)}
            {day.backups.length > 0 && <details><summary>Other options</summary>{day.backups.map((candidate, index) => <p key={index}>{candidate.room} · {candidate.start_time}–{candidate.end_time} · potential</p>)}</details>}
          </>}
          {closed(date) && eventList(date)}
        </article>;
      })}
    </div>}
