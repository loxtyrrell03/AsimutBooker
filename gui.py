...
def scheduler_health_item(
    ...
    return HealthItem(
        ...
        "ok",
        headline,
        "Every 5 minutes in the daily window; Windows wake is requested, while physical wake proof is shown separately.",
    )
...
class AsimutBookerGUI(QuietFocusGUI):
    ...
    def create_main_layout(self):
        ...
        from system_details_ui import build_system_details
        build_system_details(
            self, overview_tab, UI_COLORS, HEALTH_LABELS,
            HEALTH_STATE_SYMBOLS, HEALTH_STATE_COLORS,
        )
        # Shared display state is also consumed by Calendar and Today.
        self.booking_plan_headline_var = tk.StringVar(value="No current plan")
        self.booking_plan_detail_var = tk.StringVar(value="")
    ...
    def _apply_scheduler_status_result(self, generation, task, error):
        ...
        if error is not None:
            ...
            if not has_dashboard:
                self.tasks_status_var.set("✅ Running (5-minute schedule)")
        else:
            if not has_dashboard:
                self.tasks_status_var.set("✅ Active (every 5 min)")
    ...
    def _delete_scheduled_task(self, parent_dialog):
        ...
        if not messagebox.askyesno(
            "Remove Automatic Schedule",
            "Remove the AsimutBooker automatic 5-minute schedule?\n\n"
