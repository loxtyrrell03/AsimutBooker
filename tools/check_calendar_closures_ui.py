...
def check(dist):
    ...
    try:
        ...
        app.booking_dates = ()
        app._ensure_calendar_day_var = lambda *args, **kwargs: None
        app._render_day_cell(0, 0, date(2026, 9, 9), '2026-09-09', date(2026, 9, 8))
        cell = app.calendar_frame.winfo_children()[0]
        heading = cell.winfo_children()[0]
        assert cell.cget('bg') == '#fee2e2'
        assert heading.cget('fg') == '#b91c1c'
        assert 'overstrike' in heading.cget('font')
    finally:
        root.destroy()
    print('PASS desktop: closed date is red and crossed out')
    ...
    with tempfile.TemporaryDirectory() as temporary, sync_playwright() as playwright:
        ...
        for engine in ('chromium', 'webkit'):
            ...
            browser.close()
            print(f'PASS {engine}: empty closed day visible; open-day booking remains intact')
