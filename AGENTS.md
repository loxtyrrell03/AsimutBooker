...
1. **Pre-scan phase** (before target time):
   ...
   - Scan every live-window date and collect room candidates within the three-minute snipe window
...
- Horizon creates derive their exact unlock as slot time minus the freshly
  ...
  three-minute window, and Save exactly that block. Pending extensions run after
