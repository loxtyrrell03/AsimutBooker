...
type Section = 'goal' | 'days' | 'times' | 'rooms' | 'strategy' | 'requirements' | 'rules' | 'advance' | 'all';

const labels: Record<Section, string> = { advance: 'Advance quota', goal: 'Daily goal', days: 'Practice days', times: 'Preferred times', rooms: 'Favourite rooms', all: 'Edit practice settings', strategy: 'Booking strategy', requirements: 'Room requirements', rules: 'Booking rules' };
...
export function PracticeSettings({ csrf, enabled, onSaved, targetLabel, timeLabel, onEditing }: { onEditing?: (editing: boolean) => void; csrf: string; enabled: boolean; onSaved: () => void; targetLabel: string; timeLabel: string }) {
  ...
  async function save(event: { preventDefault: () => void }) {
    ...
    else if (section === 'advance') changes = { advance_quota: values.advance_quota };
  ...
  return <div className="practice-settings">
    ...
    {!section && <div className="preference-actions">{(['goal', 'days', 'times', 'rooms', 'requirements', 'strategy', 'advance', 'rules'] as Section[]).map(key =>
    ...
    {section && <form className="preference-editor" onSubmit={event => void save(event)} aria-label={labels[section]}>
      ...
      {values && <fieldset disabled={working || !enabled}>
        {section === 'advance' && (!values.practice_plan.enabled || !values.booking_strategy.daily_planning.enabled || values.booking_rules.preset === 'legacy') && <p>These choices apply with a daily practice goal, “Plan before booking” enabled, and New or Custom booking rules.</p>}
        {section === 'advance' && <AdvanceQuotaFields value={values.advance_quota} rooms={values.room_preferences.ordered_rooms.filter(room => !values.room_preferences.excluded_rooms.includes(room))} onChange={advance_quota => setValues({ ...values, advance_quota })} />}
...
function StrategyFields({ value, onChange }: { value: DailyPlanning; onChange: (value: DailyPlanning) => void }) {
  return <>
    <div><label>Preferred block length<select aria-label="Preferred block length" value={value.preferred_block_minutes ?? 0} onChange={event => onChange({ ...value, preferred_block_minutes: Number(event.target.value) })}>{[[0, 'Automatic'], [30, '30 minutes'], [60, '1 hour'], [90, '1½ hours'], [120, '2 hours']].map(([minutes, label]) => <option key={minutes} value={minutes}>{label}</option>)}</select></label><HelpTip label="Preferred block length">Aim for this length when equally good practice is available. Shorter or longer blocks are still booked when needed to fill your hours.</HelpTip></div>
    <div><label>Preferred rest between sessions<select aria-label="Preferred rest between sessions" value={value.preferred_rest_minutes ?? 0} onChange={event => onChange({ ...value, preferred_rest_minutes: Number(event.target.value) })}>{Array.from({ length: 9 }, (_, i) => i * 15).map(minutes => <option key={minutes} value={minutes}>{minutes ? `${minutes} minutes` : 'No preference'}</option>)}</select></label><HelpTip label="Preferred rest">Aim for this break between practice sessions, including existing bookings. Available practice takes priority when the break cannot fit.</HelpTip></div>
    <div><label><input type="checkbox" checked={value.prefer_fewer_room_changes ?? false} onChange={event => onChange({ ...value, prefer_fewer_room_changes: event.target.checked })} />Prefer fewer room changes</label><HelpTip label="Fewer room changes">Choose fewer room changes when booked hours and room/time quality are equal. Better rooms and filling your hours still come first.</HelpTip></div>
